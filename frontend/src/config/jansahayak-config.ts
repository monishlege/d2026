/**
 * JanSahayak AI System Prompt Configuration
 * 
 * Centralized system prompt used for all Gemini voice analysis endpoints.
 * Ensures consistent behavior across FastAPI backend and Vercel serverless.
 * 
 * Updated: 2026-09-01
 * Version: 2.0 (Refined for TTS optimization and scheme analysis)
 */

export const JANSAHAYAK_SYSTEM_PROMPT = `You are "JanSahayak AI", a multi-lingual digital assistant designed for Indian citizens navigating central and state government welfare schemes (such as PM-KISAN, Ayushman Bharat, e-SHRAM, PM Awas Yojana).
...
/**
 * Response Schema for TypeScript validation
 * Used with Zod for structured output generation
 */
export const JANSAHAYAK_RESPONSE_SCHEMA = {
  type: "OBJECT",
  properties: {
    detected_language: {
      type: "STRING",
      description: "Detected Language Name & BCP-47 Tag (e.g., Hindi (hi-IN))"
    },
    user_speech_transcript: {
      type: "STRING",
      description: "Exact or normalized transcript of the user's speech"
    },
    scheme_analysis: {
      type: "STRING",
      description: "1-2 sentence technical summary of eligibility or scheme matching"
    },
    spoken_response: {
      type: "STRING",
      description: "Conversational plain-text answer in the native language for TTS output"
    }
  },
...
/**
 * Supported languages for JanSahayak
 * Includes language name, native name, and BCP-47 code
 */
...
/**
 * Central Welfare Schemes Database
 * Reference for scheme eligibility analysis
 */
...
export const TTS_CONFIG = {
  ...
  maxSentences: 3,        // Maximum sentences in response
  maxCharacters: 150      // Maximum characters for TTS readability
};

/**
 * API Configuration for different deployments
 */
export const API_CONFIG = {
  fastapi: {
    endpoint: "http://localhost:8000/api/gemini-voice-analyze",
    timeout: 30000,        // 30 seconds
    description: "FastAPI Backend (Local or traditional server)"
  },
  vercel: {
    endpoint: "/api/gemini-voice-analyze",
    timeout: 30000,        // 30 seconds
    description: "Vercel Serverless (Edge deployment)"
  }
};
